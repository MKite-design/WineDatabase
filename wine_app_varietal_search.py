...
if not os.path.exists(DB_FILE):
    ...
    try:
        ...
        with st.expander("Filters", expanded=True):
            ...
            price_range = st.slider("Bottle Price Range", float(df['bottle_price'].min()), float(df['bottle_price'].max()), (10.0, 100.0))
        ...
        df = df[(df['bottle_price'] >= price_range[0]) & (df['bottle_price'] <= price_range[1])]
